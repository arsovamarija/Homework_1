...
def power_flow(U1, Un, R, X, B, P2, Q2, l):
    ...
    for _ in range(20):  # Maximum 20 iterations
        ...
        dUd, dUq = (P2 * R + Q2 * X) / U2, (P2 * X - Q2 * R) / U2 # Compute voltage drops in real and reactive components
        U2_new = math.sqrt(U1**2 - dUq**2) - dUd  # Compute new voltage

        if dec_compare(U2, U2_new): break # Check for convergence
...
def S_max(P, cosfi, U1, Un, R, X, B):
    ...
    U2_target, Qc2, S = (
        0.9 * Un,
        (B / 2) * (0.9 * Un) ** 2 / 1e6, 
        P,)  # Set target voltage, reactive compensation, and initial S
    ...
    for _ in range(1000):  # Loop up to 1000 iterations
        ...
        dUd, dUq = (
            (P2 * R + Q2 * X) / U2_target,
            (P2 * X - Q2 * R) / U2_target,) # Calculate voltage drops
